...
plot_means_km3 <- means_km3 %>% 
  gather(key = "Cluster", value = "mean", -chem, -Group) %>% as_tibble()
...
```{r}
 #K-means N=3
plot_chem_means %>% as_tibble() %>% rename(pop_mean = `colMeans(log.x)`) %>% 
...
pop_clus <- as_tibble((cbind(pop, clu.group)))
...
```{r}
health_model_kmeans5 %>% broom::tidy() %>% as_tibble() %>% filter(grepl("clu.group", term)) %>% 
...
pop_clus3 <- as_tibble((cbind(pop, clu.group3))) %>% 
...
```{r}
health_model_kmeans3 %>% broom::tidy() %>% as_tibble() %>% filter(grepl("clu.group", term)) %>% 
...
hc_health_3 <- as_tibble(cbind(pop, hc.clusters.3)) %>% 
...
```{r}
health_model_hc3 %>% broom::tidy() %>% as_tibble() %>% filter(grepl("cluster", term)) %>% 
